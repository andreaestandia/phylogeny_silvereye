...
folder_path <- file.path(reports_path, "ngsadmix", "all")

# Function to extract the number from the file name
extract_number <- function(filename) {
  as.integer(str_extract(filename, "(?<=ngsadmix_)\\d+(?=.qopt)"))
}

# List all files in the folder
file_list <- list.files(folder_path, pattern = "ngsadmix_\\d+.qopt")
...
pop_admix_levels <- c(
  ...
  "LordHoweIsland", "Heron",
  "Queensland", "NewSouthWales","Victoria",
  "WesternAustralia", "SouthAustralia",
  "Tasmania", "SouthIslandNZ", "ChathamIsland",
  "NorthIslandNZ", "NorfolkIsland", "Tahiti"
)

# Function to create the plot for each dataframe
create_plot <- function(df) {
  palette_Dark2 <- colorRampPalette(brewer.pal(18, "Dark2"))
  pal <- palette_Dark2(length(unique(df$pop_admix)))
  col<-sample(pal,length(unique(df$cluster)))
...
# Assuming you have a list of dataframes, named ngsadmix_1, ngsadmix_2, ..., ngsadmix_max_k
max_k <- 18  # Replace this with the actual maximum value of k
ngsadmix_list <- lapply(1:max_k, function(k) get(paste0("ngsadmix_", k)))
